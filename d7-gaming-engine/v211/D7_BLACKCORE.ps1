...
function BlackScan{
 Log 'BLACKSCAN started - FAST inventory mode...';[Windows.Forms.Application]::DoEvents();$ts=Get-Date -Format 'yyyyMMdd_HHmmss';$json=Join-Path $ScanRoot "D7_BLACKSCAN_$ts.json";$txt=Join-Path $ScanRoot "D7_BLACKSCAN_$ts.txt"
 ...
 $nvidia=GetNvidia;[Windows.Forms.Application]::DoEvents();
 $tools=ToolInventory;[Windows.Forms.Application]::DoEvents();
 $startup=GetStartup;[Windows.Forms.Application]::DoEvents();
 $drivers=GetDrivers;[Windows.Forms.Application]::DoEvents();
 $apps=InstalledApps;[Windows.Forms.Application]::DoEvents();
 $tasks=GetTasks;[Windows.Forms.Application]::DoEvents();
 $net=GetNet;[Windows.Forms.Application]::DoEvents();
 $hags=RegGet 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers' 'HwSchMode';$gm=RegGet 'HKCU:\Software\Microsoft\GameBar' 'AutoGameModeEnabled';$dvr=RegGet 'HKCU:\System\GameConfigStore' 'GameDVR_Enabled';$power=ActivePower
...
function ToolInventory {
    ...
    $out=@()
    foreach($d in $defs){
        $app=$apps | Where-Object { Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
[System.Windows.Forms.Application]::EnableVisualStyles()
...
LoadGames
$form=New-Object Windows.Forms.Form;$form.Text='D7 BLACKCORE v2.1.1';$form.Size=New-Object Drawing.Size(1120,720);$form.StartPosition='CenterScreen';$form.BackColor=[Drawing.Color]::FromArgb(8,10,12);$form.ForeColor=[Drawing.Color]::White
...
.DisplayName -like ('*'+$d.Match+'*') } | Select-Object -First 1
        $task=$tasks | Where-Object { Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
[System.Windows.Forms.Application]::EnableVisualStyles()
...
Log ('D7 BLACKCORE v'+$Version+' started.');Log 'Policy: reversible changes only. No HPET/BCD/timer hacks, no RealTime priority, no Defender/pagefile disabling.'
[void]$form.ShowDialog()
.TaskName -like ('*'+$d.Task+'*') -or Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
[System.Windows.Forms.Application]::EnableVisualStyles()
...
Log ('D7 BLACKCORE v'+$Version+' started.');Log 'Policy: reversible changes only. No HPET/BCD/timer hacks, no RealTime priority, no Defender/pagefile disabling.'
[void]$form.ShowDialog()
.TaskName -like ('*'+$d.N+'*') } | Select-Object -First 1
