...
fn unique_push(out:&mut Vec<String>,value:String){let v=value.trim().to_string();if!v.is_empty()&&!out.iter().any(|x|x==&v){out.push(v)}}
...
fn is_framing_signal(low:&str)->bool{
    let transport_marker=["\\\\.\\pipe\\","namedpipe","message_mode","byte_mode","recv(","readfile","writefile","send(","write("].iter().any(|k|low.contains(k));
    ...
    transport_marker||boundary_marker
}
