...
fn status(state: &State<AppState>) -> RuntimeStatus {
    RuntimeStatus { paused: state.paused.load(Ordering::SeqCst), uptime_seconds: state.started.elapsed().as_secs(), native: true, version: "2.0.0-dev.4" }
}
...
fn get_runtime_overview(state: State<AppState>) -> RuntimeOverview {
    ...
    let mut capabilities = vec![
        ...
        CapabilitySnapshot { id: "windows.write", label: "Windows Mutations", state: "untested", evidence: "Intentionally disabled until transactional adapters + real-machine verification are ready.".into() },
        CapabilitySnapshot { id: "browser.chrome", label: "Authenticated Chrome Operator", state: "untested", evidence: "Scheduled after native host milestone; no credential extraction will be used.".into() },
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_runtime_overview, set_runtime_paused])
